...
enum cpevent_t : uint8_t {
    CP_INIT, CP_DONE, CP_RESTART, CP_WEB, CP_CONNECT, CP_DISCONNECT, CP_IDLE
};

typedef std::function<void(cpevent_t event, void *param)> cpcallback_t;

struct __attribute__((__packed__)) editor_t {
    enum editortype_t : uint8_t {
        EDIT_NONE, EDIT_TEXT, EDIT_PASSWORD, EDIT_TEXTAREA, EDIT_CHECKBOX, EDIT_RADIO, EDIT_SELECT, EDIT_HIDDEN
    };
...
bool paramsCaptivePortal(Parameters *params, const char *ssid, const char *pswd, uint16_t duration = 0,
                         cpcallback_t callback = NULL);
