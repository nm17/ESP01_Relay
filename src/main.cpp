...
const paraminfo_t PARAMS[] PROGMEM = {
        ...
        PARAM_BOOL_CUSTOM(PARAM_BOOT_STATE_NAME, PARAM_BOOT_STATE_TITLE, PARAM_BOOT_STATE_DEF,
                          EDITOR_RADIO(2, BOOLS, STATES, false, false, false)),
...
static void relaySwitch(bool on, bool publish = false) {
    ...
    if (publish && mqtt && mqtt->connected()) {
        ...
        value = '0' + on;
        mqtt->publish((char *) params->value(PARAM_MQTT_TOPIC_NAME), (uint8_t *) &value, 1,
                      *(bool *) params->value(PARAM_MQTT_RETAINED_NAME));
    }
    relayState = on;
...
void setup() {
    ...
    bool paramIncomplete =
            (!*(char *) params->value(PARAM_WIFI_SSID_NAME)) || (!*(char *) params->value(PARAM_WIFI_PSWD_NAME));
            ...
            switch (event) {
                ...
                case CP_IDLE:
                    digitalWrite(LED_PIN,
                                 (millis() % (((ESP8266WiFiClass *) param)->softAPgetStationNum() > 0 ? 500 : 250) <
                                  LED_PULSE) == LED_LEVEL);
                    break;
                default:
                    break;
    ...
    if (*(char *) params->value(PARAM_MQTT_SERVER_NAME) && *(char *) params->value(PARAM_MQTT_CLIENT_NAME) &&
        *(char *) params->value(PARAM_MQTT_TOPIC_NAME)) {
        ...
        mqtt->setServer((char *) params->value(PARAM_MQTT_SERVER_NAME),
                        *(uint16_t *) params->value(PARAM_MQTT_PORT_NAME));
