        ...
        if (ptr) {
            switch (pgm_read_byte(&_params[index].type)) {
                ...
                case paraminfo_t::PARAM_BINARY:
                    ...
                    break;
                case paraminfo_t::PARAM_IP:
                    result = stream.printf_P(FMT_IP_PSTR, (*(paraminfo_t::ipaddr_t *) ptr)[0],
                                             (*(paraminfo_t::ipaddr_t *) ptr)[1],
...
String Parameters::getEditor(uint16_t index) {
    ...
    if (_inited && (index < _count) && (pgm_read_byte(&_params[index].editor.type) != editor_t::EDIT_NONE) &&
        getPtr(index)) {
        StreamString result;
        editor_t editor;
...
bool
paramsCaptivePortal(Parameters *params, const char *ssid, const char *pswd, uint16_t duration, cpcallback_t callback) {
#ifdef ESP8266
